...
An optional second job measures coverage from examples and vignettes, including
`\donttest{}` and `\dontrun{}` blocks that `R CMD check` and the unit suite skip.
A `min-coverage` input can fail the tests job below a chosen percent; it is off
by default and does not inherit a 100% bar. It does not apply to the examples
job, which measures a different execution path.
...
| `min-coverage`        | string  | `''`     | Minimum percent coverage required to pass the tests job (`covr::percent_coverage()`). Empty disables the threshold. Does not apply to the examples/vignettes job.                               |
...
jobs:
  test-coverage:
    ...
    #   min-coverage: ''             # fail the tests job below this percent; empty disables the threshold
```
