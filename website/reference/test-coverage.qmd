...
An optional second job measures coverage from examples and vignettes.
`\dontrun{}` never runs under `R CMD check`.
`\donttest{}` is skipped unless `--as-cran` or `--run-donttest`.
covr comments both out by default;
the examples job passes `commentDonttest`/`commentDontrun` false so those
blocks execute for coverage.
A `min-coverage` input can fail the tests job below a chosen percent;
it defaults to no threshold and does not apply to the examples
