...
| `report-cost` | boolean | `true` | Append the run's dollar cost (`total_cost_usd`) to whichever comment this workflow posts back to the thread. The action itself only writes this to the job step summary, never a comment. Set false to suppress it. |
