...
| `use-ai-config`          | boolean | `true`                      | Install the [`Morrison-Lab/ai-config`](https://github.com/Morrison-Lab/ai-config) plugin (`ai-config@d-morrison`), giving the agent the lab's shared skills, slash commands, and standing workflow conventions. Set false to opt out. Distinct from `link-skills`, which exposes the calling repo's own `skills/` directory; the two compose.  |
| `plugin-marketplaces`    | string  | `''`                        | Extra newline-separated Claude Code plugin marketplace Git URLs, added on top of the `d-morrison/ai-config.git` marketplace `use-ai-config` adds. Pair with `plugins` below; empty adds none.                                                                                                                                        |
...
jobs:
  claude:
    ...
    #   use-ai-config: false        # skip the d-morrison/ai-config plugin (installed by default)
