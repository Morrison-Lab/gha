...
after punctuation, or after a polite lead-in -- `@claude review`,
`@claude, review`, `@claude please review`, `@claude can you review this?`,
`@claude re-review`. Anything else is treated as an ordinary request for the
agent, including a mention that merely contains the word (`@claude the review
workflow is broken, can you fix it?`), because a misfire routes the comment to
the reviewer and suppresses the agent's own reply. Quoted lines are skipped, so
quote-replying to a past request does not dispatch a fresh review.
