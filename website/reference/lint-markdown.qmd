...
code blocks longer than a configurable line threshold, list-item merge
splices, and blank lines that split a GFM table.
...
| `paths-ignore`             | string  | `''`     | Comma/newline-separated glob patterns (relative paths) to skip, for markdownlint and companion checks. Supports `*`, `?`, and recursive `**`.                                    |
...
| `base-ref`                 | string  | `''`     | Git ref/SHA to diff against. If set, only newly added lines in the diff are checked for list-item merge splices. Defaults to empty (skips check to avoid whole-tree false-positives). Pass 'all' to scan all lines. |
| `fail-on-item-splices`     | boolean | `true`   | Fail the workflow when a list-item merge splice is detected.                                                                                                                                                     |
| `fail-on-table-splits`     | boolean | `true`   | Fail the workflow when a blank line splits a GFM table, orphaning the rows below it so they render as literal text. Scans the whole tree rather than diff-scoping, since a split table has no legitimate form. |
...
jobs:
  lint:
    ...
    #   fail-on-table-splits: true          # fail when a blank line splits a GFM table
```
