...
If you have disabled `claude.yml`, set the repository variable
`CLAUDE_AGENT_DISABLED` to `true` so the stub's `dispatch-on-comment` job also
accepts an `@claude review` mention. Without it a repo with the agent off has no
...
# If you have DISABLED claude.yml, also set the repository variable
# CLAUDE_AGENT_DISABLED to `true`, so an `@claude review` mention dispatches a
# review here rather than reaching a workflow that no longer answers it.
