...
If the caller also enables a `pull_request` trigger, it can run automatically on PR updates.
Note that GitHub will not trigger `pull_request` workflow runs if the pull request has merge conflicts with the target branch (GitHub Actions platform behaviour; gha#859).
A direct `/review` path is available too, but only if the caller also enables
`issue_comment`.
`claude.yml` dispatches it via `workflow_dispatch`; keep the
...
`actions: read`, `checks: read` (grant it; required by the
currently-tagged `@v2`),
