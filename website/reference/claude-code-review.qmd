...
|---|---|---|---|
| `pr-number` | string | `''` | PR number to review (set by `claude.yml`'s `workflow_dispatch`; empty unless the caller also enables `pull_request` runs). |
...
jobs:
  review:
    ...
    with:
      ...
      # a review on Claude's commits; empty (and ignored) unless pull_request is enabled.
