...
| `track-progress`      | boolean | `false`                 | Ignored (gha#580). Tag mode needs a writable forge token in the model job. This workflow posts from a separate job instead. The input is kept so existing callers do not fail at the call gate. |
...
context-gathering job having been cancelled or failed --- plus three cases
...
- `post-review` marked the payload stale (the PR head moved, or the reviewed
  SHA could not be bound) so a newer run owns the thread.
