...
By default, callers run it only on dispatched runs (for `@claude review` via
`claude.yml`), which posts a consolidated summary.
If the caller also enables a `pull_request` trigger,
it can run automatically on PR updates.
GitHub suppresses `pull_request` workflow runs
when a pull request has merge conflicts with the target branch (gha#859).
A direct `/review` path is available too,
but only if the caller also enables `issue_comment`.
`claude.yml` dispatches it via `workflow_dispatch`;
keep the stub named `claude-code-review.yml`
(or set `claude.yml`'s `review-workflow-file` to match)
...
| `use-posit-skills`    | boolean | `true`                  | Install the [`posit-dev/skills`](https://github.com/posit-dev/skills) plugin (`posit-dev@posit-dev-skills`) alongside the built-in `code-review@claude-code-plugins` and `ai-config@Morrison-Lab`, so the reviewer applies Posit's critical code review guidelines (including `critical-code-reviewer`). Set false to opt out.                                                                                                                                                                            |
| `reference-repos`     | string  | `'Morrison-Lab/gha'`    | Space-, comma-, or newline-separated list of GitHub repositories (`owner/repo` or `owner/repo@ref`) checked out into `.reference-repos/` so the reviewer can inspect their files and schemas directly. Set to `'none'` or `''` to disable.                                                                                                                                                                                                                                           |
...
| `plugin-marketplaces` | string  | `''`                    | Extra newline-separated plugin marketplace Git URLs, added on top of the marketplaces this workflow already uses (`anthropics/claude-code.git` for the `code-review` plugin, `Morrison-Lab/ai-config.git` unless `use-ai-config` is false, and `posit-dev/skills.git` unless `use-posit-skills` is false). Pair with `plugins` below; empty adds none.                                                                                                                                                                                                         |
| `plugins`             | string  | `''`                    | Extra newline-separated plugin refs to install (`<plugin>@<marketplace-name>`), added on top of the built-in `code-review@claude-code-plugins`, `ai-config@Morrison-Lab`, and `posit-dev@posit-dev-skills`. Each marketplace name must match the name that marketplace declares in its own `.claude-plugin/marketplace.json`, not the URL given in `plugin-marketplaces`.                                                                                                                                                  |
...
`actions: read`, `checks: read` (grant it; optional under `@v2`,
which no longer requests it per gha#832,
but required by the model job as of `@v3`),
and add either the `CLAUDE_CODE_OAUTH_TOKEN`
...
A files-list response shorter than the PR's `changed_files` count,
or reaching the 3000-file endpoint cap (gha#917),
is treated the same as a failed listing (GitHub caps that endpoint at 3000
...
jobs:
  review:
    permissions:
      ...
      issues: write
      actions: read
      checks: read    # keep: required by @v2 until the slide, and again at the v3 (gha#833)
    ...
    #   reference-repos: 'Morrison-Lab/gha'  # check out reference repos into .reference-repos/ for the reviewer
