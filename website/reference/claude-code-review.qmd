...
`actions: read`, and add either the `CLAUDE_CODE_OAUTH_TOKEN` or
...
One gray result is a canceled review: a `claude-review` run canceled by a
newer run of the same PR (`cancel-in-progress`) skips this gate
([gha#585](https://github.com/Morrison-Lab/gha/issues/585)).
Check whether `claude-review`'s conclusion is `cancelled` rather than
`failure` before investigating the diff, and trust the surviving run's
review.

The gate skips gray whenever no review was attempted. That covers anything
that made the `claude-review` job's own `if:` false --- a draft, fork, or
bot-authored PR, a dispatch blocked by the fork/Dependabot guard, or the
context-gathering job having been cancelled --- plus three cases
where a job ran and the verdict was withheld:
...
- `post-review` marked the payload stale because the PR head moved, so a
  newer run owns the thread.

A gather-context *failure* is not a gray skip: the model still runs, and
an unbound reviewed SHA (`stale=true` plus `post-review` exiting 1) turns
this gate **red**. That is not "a newer run owns the thread"; there may
be no successor. The same red applies to a live-head lookup that cannot
read the PR SHA.

That self-mod case deserves care. This workflow's skip guard refuses to
review a PR that edits the caller review workflow; **no re-run or
re-dispatch will change that** --- the skip lifts only if the PR stops
editing the file, which is rarely what the PR is for.
Forwarding `github_token` skips `claude-code-action`'s OIDC App-token
exchange, so that action's "workflow must match the default branch"
validation is not what enforces the skip (gha#580).
The review job posts a PR comment saying so. Merge such a PR on a
self-review or a human review; do not read the gray gate as an all-clear.

## Concurrency

Do not declare a top-level `concurrency:` block in your caller workflow.
`claude-code-review.yml` manages per-PR concurrency internally on two
groups:

- `claude-review-<PR>` (`cancel-in-progress: true`) --- `preempt-previous`
  and `claude-review`. Queuing `preempt-previous` cancels a predecessor
  model job. It does not wait for that run's restore.
- `claude-review-stash-<PR>` (`cancel-in-progress: false`) ---
  `gather-context` and `post-review`. Writers queue rather than cancel
  each other.

`require-review` is in neither group.
