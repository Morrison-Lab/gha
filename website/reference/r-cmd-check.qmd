...
| `julia-project` | string | `'inst/julia'` | Path to the Julia project to instantiate when `setup-julia` is true. |
| `apt-packages` | string | (common build deps) | Space-separated apt packages to install before R setup on `ubuntu-latest` runner cells without a container, and on the hard job. Pass empty string to skip. |
| `brew-packages` | string | `''` | Space-separated Homebrew packages to install on `macos-*` runner cells before R setup. When `gettext` is included, it is force-linked and added to `~/.R/Makevars`. |
| `brew-casks` | string | `''` | Space-separated Homebrew casks to install on `macos-*` runner cells before R setup (e.g. `xquartz`). |
...
[`ucdavis/bcs`](https://github.com/ucdavis/bcs)'s bespoke
`.github/workflows/R-CMD-check.yaml` and its
`.github/actions/install-system-deps` composite can migrate directly to
this reusable workflow with `setup-julia: true`, `brew-packages: gettext`,
`brew-casks: xquartz`, and default `apt-packages`.

| bcs step | This workflow |
|---|---|
| `.github/actions/install-system-deps` (apt packages on Linux) | Default `apt-packages` matches bcs's package list exactly. |
| `brew install gettext` and `brew link gettext --force` on macOS | `brew-packages: gettext`. Force-link and `~/.R/Makevars` configuration run automatically. |
| `brew install --cask xquartz` on macOS | `brew-casks: xquartz`. |
| `julia-actions/setup-julia@v2` | `setup-julia: true`. |
| `julia-actions/cache@v2` | Automatic when `setup-julia: true` and `julia-project != ''` (pinned to v3.3.0). |
| `Pkg.instantiate()` / `Pkg.precompile()` on `inst/julia` | `julia-project: inst/julia` (the default when `setup-julia: true`). |
| `PKG_INCLUDE_LINKINGTO: "true"` in job env | Always set in both jobs' env. |
...
jobs:
  R-CMD-check:
    ...
    #   apt-packages: ''              # empty to skip; default installs build deps
    #   brew-packages: gettext        # bcs macOS deps
    #   brew-casks: xquartz
    #   install-quarto: true
