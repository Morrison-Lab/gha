...
| (unset) `_R_CHECK_CRAN_INCOMING_REMOTE_`, `_R_CHECK_FORCE_SUGGESTS_`, `_R_CHECK_STOP_ON_INVALID_NUMERIC_VERSION_INPUTS_` | Inputs. `cran-incoming-remote` also sets `_R_CHECK_CRAN_INCOMING_`, because `r-lib/actions/check-r-package` forces INCOMING false when unset (REMOTE-only cannot enable incoming checks). Defaults follow [issue #331](https://github.com/Morrison-Lab/gha/issues/331)'s description of IndrajeetPatil (`false`, `false`, `true`). rpt does not set these either; r-lib then sets `_R_CHECK_FORCE_SUGGESTS_=false` when unset, so this workflow's `force-suggests: false` already matches rpt. |
