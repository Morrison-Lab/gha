...
In the `claude-code-review` workflow, when the denied-tools sidecar executes, it attempts to write output to `$GITHUB_OUTPUT` using a heredoc.
If the output lacks a trailing newline, the heredoc remains unterminated, causing the workflow step to error out.
As a result, the workflow never reaches the job that actually posts the failure comment to the pull request.
