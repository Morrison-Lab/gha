...
The `check-new-line-breaks` tool currently ignores `.qmd` and `.yml` files because its default glob pattern (`NLB_GLOBS`) only targets `*.md`, and no caller overrides this default.
This leaves Quarto documents and YAML workflows un-scanned for semantic line breaks.
