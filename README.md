...
- `bump-dev-version` (opens/auto-merges a PR) → grant `contents: write`,
  ...
  checks never report; see the reference page). A fine-grained PAT needs
  **Pull requests: Read and write** on that secret in addition to **Contents:
  write**; the workflow probes pull-request access before any bump work. `version-check` (read-only) →
