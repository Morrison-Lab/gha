...
| `check-dependency-updates.yml` | Automated dependency freshness audit: mine release notes for high-value updates and file discovered opportunities as issues | `target-repo`, `dry-run`, `max-issues`, `labels`, `model`, `use-ai-config`, `timeout-minutes` |
...
- `check-dependency-updates` (audits dependencies and files update issues) →
  grant `contents: read`, `issues: write`, and pass either
  `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY`.
