...
| Dollar cost of the run | `total_cost_usd` (on the execution output's `result` event) | Yes, indirectly — the action itself only writes this to the job step summary, never a comment. Both workflows extract it and post it in a comment instead, gated on the `report-cost` input (default `true`). |
