- **Documented the `anthropics/claude-code-action` workflow-validation skip**
  ...
  If that guard were bypassed -- as gha#417 proposed and this repo rejected --
  the action's own content-validation fires instead: it gracefully skips,
  `check-review-execution.sh` sees no execution output, and
