- **`lint-markdown` gains a split-table check and a `fail-on-table-splits`
  ...
  structurally cannot see this -- which is how `README.md`'s capability table
  carried the defect from `cacf1df` until #555, visible only to a person
  opening the file on GitHub.
