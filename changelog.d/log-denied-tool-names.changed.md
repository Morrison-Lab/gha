- **`claude-code-review`'s guard now names the denied tools, not just how many
  ...
  Token-shaped literals in the sample are redacted, and a result carrying a
  count but no array says so explicitly instead of printing an empty list.
  The summarizer also tolerates a malformed denial entry, which previously
  aborted the whole script before it classified the review at all.
