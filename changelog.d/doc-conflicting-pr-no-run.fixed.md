- **Document that GitHub suppresses `pull_request` workflow runs on conflicting PRs** (#859).
  ...
  GitHub Actions runs no `pull_request` workflows on push or ready events.
  The documentation in `claude-code-review.yml`,
  `examples/claude-code-review.yml`,
  `README.md`,
  `website/workflows.qmd`,
  `website/reference/claude-code-review.qmd`,
  and `CLAUDE.md` now details this platform behavior
  and notes that `workflow_dispatch` can be used to trigger
