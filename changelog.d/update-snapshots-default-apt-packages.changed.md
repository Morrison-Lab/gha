- **`update-snapshots` now defaults `apt-packages` to the standard R
  ...
  install step as before. This lets [`ucdavis/bcs`](https://github.com/ucdavis/bcs)
  drop the package list it currently duplicates across four call sites, since a
  local composite action can't be shared with an external `workflow_call`
  workflow (ucdavis/bcs#229).
