- `claude.yml`: a new `dispatch-on-assignee` input makes assigning an issue a
  ...
  it needs no content signal the way `opened` does. The input takes a JSON array
  of logins and is empty by default, so nothing changes for a consumer that does
  not opt in.
  ...
  Purely additive. An assigned issue whose body or title mentions `@claude`
  already dispatched before this and still does.
