- **`claude-code-review` resolves the reviewed commit once, so the
  ...
  ephemeral merge ref** (#852).
  The model job now derives `REVIEWED_COMMIT`
  ...
  leaving a placeholder the reviewer filled from `GITHUB_SHA`.
  The structured field and the trailing `Reviewed commit:` line therefore
  come from the same resolution and cannot disagree, and
  `run-review-job-split-tests.py` pins each hop, including that nothing
  hands the prompt `github.sha`.
