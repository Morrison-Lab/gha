- Documented the required caller `permissions:` grant for every
  consumer-facing reusable workflow that was missing from the Permissions
  docs (`ai-code-review`, `gemini`, `gemini-code-review`,
  `antigravity-code-review`, `small-model-agent`, `claude-manage-project`,
  `altdoc-multiversion-docs`), and adopted the convention that widening a
